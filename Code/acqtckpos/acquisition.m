function  [Acquired,NotAcquired] = acquisition(file,signal,acq)
%Purpose:
...
NotAcquired.SNR = [];
NotAcquired.Doppler = [];
NotAcquired.codedelay = [];
NotAcquired.fineFreq = [];
%% read data
...
for svindex = 1:32
    ...
    if SNR >= 18  % acquisition thredhold
        ...
        NotAcquired.sv        = [NotAcquired.sv        svindex];
        NotAcquired.SNR       = [NotAcquired.SNR       SNR];
        NotAcquired.Doppler   = [NotAcquired.Doppler   Doppler];
        NotAcquired.codedelay = [NotAcquired.codedelay codePhase-1];
    end
end
